...
vector<vector<double>> ASHP_Para_Set(const vector<double>& input) {
	...
	para[9] = 1005.5; // cp,eva (J/[kg¡¤K], under -10¡æ)
	para[10] = 992.22; // ¦Ñcon (kg/m3, under 40¡æ)
	para[11] = 1.3248; // ¦Ñeva (kg/m3, under -10¡æ)
	para[12] = 2.0; // ¦¤Tcon,ref (K), require modeling ?
	para[13] = 5.0; // ¦¤Teva,ref (K), require modeling ?
...
vector<double> ASHP_AE_Fun(const vector<double>& x, const vector<double>& K) {
...
vector<double> ASHP_Var_Derive(const vector<double>& para, const vector<double>& sol) {
	vector<double> Re(7);
...
tuple<vector<string>, vector<double>> ASHP_Sim(const vector<double>& input, bool start, vector<double> x0_pre) {
	vector<double> R;
	vector<vector<double>> Re1 = ASHP_Para_Set(input);
	vector<vector<double>> Re2;
	vector<double> x0 = { 6.0 };

	if (start) {
		Re2 = AE_Solve_AHS(ASHP_AE_Fun, 20, x0, Re1[1], false);
	}
	else {
		Re2 = AE_Solve_Basic(ASHP_AE_Fun, x0_pre, Re1[1]);
	}
	
	vector<double> Re3 = ASHP_Var_Derive(Re1[0], Re2[0]);
