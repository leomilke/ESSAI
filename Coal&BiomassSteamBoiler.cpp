...
vector<double> CBSB_Para_Set(const vector<double>& input) {
...
vector<double> CBSB_ODE_Fun(const vector<double>& x, double h, const vector<double>& K) {
	vector<double> dx(x.size());
	double E = 7.9e4, R = 8.314;
	dx[0] = K[0] * 0.1 * (550.0 / 6.0e-8) * exp(-E / R / x[0]) * x[2] - K[1] * (550.0 / 350.0) * (x[0] - x[1]); // T
	dx[1] = K[2] * (150.0 / 350.0) * (x[0] - x[1]); // Tst
	dx[2] = -0.9 * (0.21 / 6.0e-8) * exp(-E / R / x[0]) * x[2]; // fO2

	return dx;
}

vector<double> CBSB_AE_Fun(const vector<double>& K, const vector<double>& para) {
	vector<double> f(K.size());
	vector<vector<double>> R;
	vector<double> x0 = { para[5], para[3], 0.21 };
	double range_boiler[2] = { 0.0, 1.0 };

	R = ODE_Solve_Euler_Forward(CBSB_ODE_Fun, range_boiler, 0.001, x0, K);
...
tuple<vector<string>, vector<double>> CBSB_Sim(const vector<double>& input, bool start, vector<double> T_b_b0) {
	vector<double> R;
	vector<double> Re1 = CBSB_Para_Set(input);
	...
	if (start) {
		K_combust_boiler.push_front(AE_Solve_PSO(CBSB_AE_Fun, 5.0, 3, 1.0e-3, Re1, 0.01)[0]);
		Load_pre.push_front(-1.0);
		cout << "CB Steam Boiler Init completed !" << endl;
	}

	if (Re1[8] == Load_pre.front()) {
		T_b_b_op = T_b_b0[0];
		x0[0] = T_b_b_op;
		Para_inside_op = ODE_Solve_Euler_Forward(CBSB_ODE_Fun, range_boiler, 0.001, x0, K_combust_boiler.front());
		T_b_o = *(Para_inside_op[1].end() - 1);
	}
	else {
		for (double T_b_b = 400.0 + 273.15; T_b_b <= Re1[5]; ++T_b_b) {
			x0[0] = T_b_b;
			Para_inside = ODE_Solve_Euler_Forward(CBSB_ODE_Fun, range_boiler, 0.001, x0, K_combust_boiler.front());
...
vector<string> CBSB_simple_name = {
